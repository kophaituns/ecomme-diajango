...
def admin_add_product_view(request):
    if request.method == 'POST':
        ...
        if productForm.is_valid():
            ...
            # Show form with errors and retain user input
...
# View for the AI chatbot
@login_required(login_url='customerlogin')
def ai_chatbot_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    chat_history = models.ChatMessage.objects.filter(customer=customer).order_by('-timestamp')[:10]
    chat_form = forms.ChatMessageForm()
    
    if request.method == 'POST':
        chat_form = forms.ChatMessageForm(request.POST)
        if chat_form.is_valid():
            message = chat_form.cleaned_data['message']
            
            # Get AI response
            ai_response = ai_services.get_chatbot_response(message, customer)
    ...
    # If AJAX request for chat history
    ...
    # For normal GET requests, render the chat page
    # for cart counter
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        counter = product_ids.split('|')
    ...
    return render(request, 'ecom/ai_chatbot.html', {
        'chat_form': chat_form, 
        'chat_history': chat_history,
        'product_count_in_cart': product_count_in_cart
    })

# API endpoint for AI chat (for AJAX calls)
@login_required(login_url='customerlogin')
...
# View for AI product recommendations
@login_required(login_url='customerlogin')
def ai_recommendations_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    
    # Get order history to build user preferences
    ...
    # Build user preferences based on order history
    user_preferences = f"Customer has previously purchased: {', '.join([p.name for p in ordered_products])}" if ordered_products else "New customer with no purchase history"
    
    # Get all available products
    all_products = models.Product.objects.all()
    
    # Get AI recommendations
    recommended_product_ids = ai_services.get_product_recommendations(user_preferences, all_products)
    
    # Get recommended products
    recommended_products = models.Product.objects.filter(id__in=recommended_product_ids)
    ...
    return render(request, 'ecom/ai_recommendations.html', {
        'recommended_products': recommended_products,
        'product_count_in_cart': product_count_in_cart
    })

# AI-powered feedback analysis for admin
@login_required(login_url='adminlogin')
def ai_analyze_feedback_view(request, pk):
    feedback = models.Feedback.objects.get(id=pk)
    
    # Get AI analysis
    analysis = ai_services.analyze_product_feedback(feedback.feedback)
    
    return render(request, 'ecom/ai_feedback_analysis.html', {
        'feedback': feedback,
        'analysis': analysis
    })
